sap.ui.define([
	...
	return BaseObject.extend("be.ehb.tftw.model.GroupSortState", {
		 ...
		 * @alias be.ehb.tftw.model.GroupSortState
		 */
		...
		/**
		 * Sorts by messageType, or by longitude
		 *
		...
		/**
		 * Groups by longitude, or resets the grouping for the key "None"
		 *
		 * @param {string} sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function (sKey) {
			...
			if (sKey === "longitude") {
				...
				this._oViewModel.setProperty("/sortBy", "longitude");

				aSorters.push(
					new Sorter("longitude", false,
						this._fnGroupFunction.bind(this))
				);
			} else if (sKey === "None") {
				// select the default sorting again
				this._oViewModel.setProperty("/sortBy", "messageType");
			}
