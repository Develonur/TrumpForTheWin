sap.ui.define([
		...
		function getFrameUrl (sHash, sUrlParameters) {
			var sUrl = jQuery.sap.getResourcePath("be/ehb/tftw/app", ".html");
			sHash = sHash || "";
			sUrlParameters = sUrlParameters ? "?" + sUrlParameters : "";

			if (sHash) {
				sHash = "#TrumpForTheWin-display&/" + (sHash.indexOf("/") === 0 ? sHash.substring(1) : sHash);
			} else {
				sHash = "#TrumpForTheWin-display";
			}

			return sUrl + sUrlParameters + sHash;
		}

		return Opa5.extend("be.ehb.tftw.test.integration.pages.Common", {
			...
			getMockServer : function () {
				return new Promise(function (success) {
					Opa5.getWindow().sap.ui.require(["be/ehb/tftw/localService/mockserver"], function (mockserver) {
